...
const EVENT_COLORS: Record<TipoEvento, string> = {
  Partido: '#15803d',
  Entrenamiento: '#ea7a16',
  Descanso: '#7b8494',
  Otro: '#2474b5',
};
const EVENT_COLOR_LEGEND: TipoEvento[] = ['Partido', 'Entrenamiento', 'Descanso', 'Otro'];
...
export default function Calendario() {
  ...
  const initialActaEvento = state.actaEventoPendienteId === null
    ? null
    : initialData.eventos.find((item) => item.id === state.actaEventoPendienteId && item.equipoId === equipoId && item.tipo === 'Partido') || null;
  ...
  useEffect(() => { if (hydrated) writeSportsCalendarData({ eventos, actas, prefs: initialData.prefs, deletedEquipoIds: initialData.deletedEquipoIds }); }, [eventos, actas, hydrated, initialData.deletedEquipoIds, initialData.prefs]);
  useLayoutEffect(() => {
    if (state.actaEventoPendienteId === null) return;
    const evento = eventos.find((item) => item.id === state.actaEventoPendienteId && item.equipoId === equipoId && item.tipo === 'Partido');
  ...
  }, [actions, equipoId, eventos, state.actaEventoPendienteId]);
  ...
  const openEditor = (fecha?: string, evento?: Evento) => setEditor(evento || { id: Date.now(), esNuevo: true, equipoId, tipo: 'Entrenamiento', fecha: fecha || inicioMes, horaInicio: '19:00', horaFin: '21:00' });
  const saveEvent = (evento: Evento, scope: 'uno' | 'serie' = evento.ocurrenciaDe && window.confirm('¿Aplicar los cambios a toda la serie? Aceptar = toda la serie; Cancelar = solo este evento.') ? 'serie' : 'uno') => {
    const isLeagueMatch = evento.tipo === 'Partido' && (evento.competencia || 'Liga') === 'Liga';
    const isCupMatch = evento.tipo === 'Partido' && evento.competencia === 'Copa';
    const hasValidMatchday = typeof evento.numeroFecha === 'number' && Number.isInteger(evento.numeroFecha) && evento.numeroFecha > 0;
    const hasValidCupStage = evento.instancia !== undefined && INSTANCIAS_COPA.includes(evento.instancia);
    if (!evento.fecha || !evento.equipoId || (evento.tipo === 'Partido' && (!evento.rival?.trim() || !evento.horaInicio)) || (isLeagueMatch && !hasValidMatchday) || (isCupMatch && !hasValidCupStage) || (evento.tipo === 'Otro' && !evento.titulo?.trim()) || (evento.recurrencia && (!evento.recurrencia.dias.length || !evento.recurrencia.hasta || evento.recurrencia.hasta < evento.fecha))) { actions.mostrarToast('Completá los campos obligatorios'); return; }
    const normalized = { ...evento, equipoId, numeroFecha: isLeagueMatch ? evento.numeroFecha : undefined, instancia: isCupMatch ? evento.instancia : undefined, titulo: evento.titulo?.trim(), rival: evento.rival?.trim(), descripcion: evento.descripcion?.trim() };
  ...
  return <div className="sports-calendar-screen" style={{ animation: 'fadeIn .3s ease' }}>
    <header style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 18 }}>
      <div><h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#16203a' }}>Calendario</h1><p style={{ color: '#6b7488', margin: '4px 0 0' }}>Agenda de {equipo?.nombre || 'este plantel'}</p></div>
      <button className="calendar-primary" onClick={() => openEditor()}>+ Nuevo evento</button>
    </header>
    <div aria-label="Referencia de colores de actividades" style={{ display: 'flex', alignItems: 'center', gap: '8px 16px', flexWrap: 'wrap', margin: '-6px 0 18px', color: '#566176', fontSize: 12, fontWeight: 700 }}>
      {EVENT_COLOR_LEGEND.map((tipo) => <span key={tipo} style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><i aria-hidden="true" style={{ width: 9, height: 9, borderRadius: '50%', background: EVENT_COLORS[tipo] }} />{tipo}</span>)}
    </div>
    ...
    {editor && <EventEditor evento={editor} onChange={setEditor} onSave={saveEvent} onDelete={() => deleteEvent(editor, editor.ocurrenciaDe && window.confirm('¿Eliminar toda la serie? Aceptar = toda la serie; Cancelar = solo este evento.') ? 'serie' : 'uno')} onOpenActa={() => { setActaEvento(editor); setEditor(null); }} onClose={() => setEditor(null)} />}
...
function MonthView({ year, month, eventos, actas, color, onNew, onEvent, onMore }: { year: number; month: number; eventos: Evento[]; actas: Record<string, Acta>; color: (tipo: TipoEvento) => string; onNew: (fecha: string) => void; onEvent: (evento: Evento) => void; onMore: (fecha: string) => void }) {
  const monthFirst = new Date(year, month, 1).getDay(); const monthDays = new Date(year, month + 1, 0).getDate();
  const renderMonthEvent = (evento: Evento) => { const result = resultadoPartido(evento, actas); return <button key={String(evento.id)} className={`sports-event${result ? ` match-${result.clase}` : ''}`} style={{ borderLeftColor: color(evento.tipo) }} onClick={() => onEvent(evento)}>{result ? <span className="match-result-chip">{result.club}–{result.rival} vs {evento.rival || 'Rival a definir'}</span> : <><span>{evento.horaInicio || 'Todo el día'}</span>{eventTitle(evento)}</>}</button>; };
...
function WeekView({ start, eventos, actas, color, onEvent }: { start: Date; eventos: Evento[]; actas: Record<string, Acta>; color: (tipo: TipoEvento) => string; onEvent: (evento: Evento) => void }) {
  const weekDays = Array.from({ length: 7 }, (_, index) => addDays(start, index));
  const renderWeekEvent = (evento: Evento) => { const result = resultadoPartido(evento, actas); return <button className={`sports-week-event${result ? ` match-${result.clase}` : ''}`} key={String(evento.id)} style={{ borderLeftColor: color(evento.tipo) }} onClick={() => onEvent(evento)}>{result ? <span className="match-result-chip">{result.club}–{result.rival} vs {evento.rival || 'Rival a definir'}</span> : <><span>{evento.horaInicio || 'Todo el día'}</span>{eventTitle(evento)}</>}</button>; };
...
function DayDetailWithResults({ fecha, eventos, actas, color, onClose, onEvent, onNew }: { fecha: string; eventos: Evento[]; actas: Record<string, Acta>; color: (tipo: TipoEvento) => string; onClose: () => void; onEvent: (evento: Evento) => void; onNew: () => void }) {
  return <div className="calendar-overlay" onClick={onClose}><div className="calendar-day-detail" onClick={(event) => event.stopPropagation()}><button className="calendar-close" onClick={onClose}>×</button><h2>Agenda del {fecha.split('-').reverse().join('/')}</h2>{eventos.map((evento) => { const result = resultadoPartido(evento, actas); return <button key={String(evento.id)} className={`day-detail-event${result ? ` match-${result.clase}` : ''}`} style={{ borderLeftColor: color(evento.tipo) }} onClick={() => { onClose(); onEvent(evento); }}><strong>{result ? `${eventTitle(evento)} · ${result.club} — ${result.rival}` : `${evento.horaInicio || 'Todo el día'} · ${eventTitle(evento)}`}</strong><span>{evento.tipo}</span></button>; })}<button className="calendar-primary" onClick={onNew}>+ Nuevo evento</button></div></div>;
}

function EventEditor({ evento, onChange, onSave, onDelete, onOpenActa, onClose }: { evento: Evento; onChange: (event: Evento) => void; onSave: (event: Evento) => void; onDelete: () => void; onOpenActa: () => void; onClose: () => void }) {
  const patch = (values: Partial<Evento>) => {
    const next = { ...evento, ...values };
    const competencia = next.competencia || 'Liga';
    onChange({
      ...next,
      numeroFecha: next.tipo === 'Partido' && competencia === 'Liga' ? next.numeroFecha : undefined,
      instancia: next.tipo === 'Partido' && competencia === 'Copa' ? next.instancia : undefined,
    });
  };
  const recurrente = !!evento.recurrencia;
  const isLeagueMatch = evento.tipo === 'Partido' && (evento.competencia || 'Liga') === 'Liga';
  const isCupMatch = evento.tipo === 'Partido' && evento.competencia === 'Copa';
  const updateMatchday = (value: string) => {
    if (value === '') { patch({ numeroFecha: undefined }); return; }
    const numeroFecha = Number(value);
    if (Number.isInteger(numeroFecha) && numeroFecha > 0) patch({ numeroFecha });
  };
  return <div className="calendar-overlay"><aside className="calendar-drawer"><div className="drawer-header"><h2>{evento.esNuevo ? 'Nuevo evento' : 'Editar evento'}</h2><button className="calendar-close" onClick={onClose}>×</button></div><label>Tipo<select value={evento.tipo} onChange={(e) => { const tipo = e.target.value as TipoEvento; patch({ tipo, recurrencia: undefined }); }}>{(['Partido', 'Entrenamiento', 'Descanso', 'Otro'] as TipoEvento[]).map((tipo) => <option key={tipo}>{tipo}</option>)}</select></label><div className="drawer-grid"><label>Fecha<input type="date" value={evento.fecha} onChange={(e) => patch({ fecha: e.target.value })} /></label>{evento.tipo !== 'Descanso' && <label>Hora inicio{evento.tipo === 'Partido' && ' *'}<input type="time" value={evento.horaInicio || ''} onChange={(e) => patch({ horaInicio: e.target.value })} /></label>}{evento.tipo === 'Entrenamiento' && <label>Hora fin<input type="time" value={evento.horaFin || ''} onChange={(e) => patch({ horaFin: e.target.value })} /></label>}</div>{evento.tipo === 'Partido' && <><label>Rival *<input value={evento.rival || ''} onChange={(e) => patch({ rival: e.target.value })} /></label><div className="drawer-grid"><label>Competencia<select value={evento.competencia || 'Liga'} onChange={(e) => patch({ competencia: e.target.value as Competencia })}><option>Liga</option><option>Copa</option><option>Amistoso</option></select></label><label>Condición<select value={evento.condicion || 'Local'} onChange={(e) => patch({ condicion: e.target.value as 'Local' | 'Visitante' })}><option>Local</option><option>Visitante</option></select></label></div>{isLeagueMatch && <label>Número de fecha *<input type="number" min="1" step="1" inputMode="numeric" required value={evento.numeroFecha ?? ''} onChange={(e) => updateMatchday(e.target.value)} /></label>}{isCupMatch && <label>Instancia *<select required value={evento.instancia || ''} onChange={(e) => patch({ instancia: e.target.value ? e.target.value as InstanciaCopa : undefined })}><option value="" disabled>Seleccionar instancia</option>{INSTANCIAS_COPA.map((instancia) => <option key={instancia} value={instancia}>{instancia}</option>)}</select></label>}</>}{evento.tipo === 'Otro' && <label>Título *<input value={evento.titulo || ''} onChange={(e) => patch({ titulo: e.target.value })} /></label>}{evento.tipo === 'Entrenamiento' && <label>Objetivo / título<input value={evento.titulo || ''} onChange={(e) => patch({ titulo: e.target.value })} /></label>}{evento.tipo !== 'Descanso' && <label>Lugar<input value={evento.lugar || ''} onChange={(e) => patch({ lugar: e.target.value })} /></label>}<label>Descripción / observaciones<textarea value={evento.descripcion || ''} onChange={(e) => patch({ descripcion: e.target.value })} /></label>{(evento.tipo === 'Entrenamiento' || evento.tipo === 'Descanso') && <div className="recurrence"><label className="check-label"><input type="checkbox" checked={recurrente} onChange={(e) => patch({ recurrencia: e.target.checked ? { dias: [dateFromIso(evento.fecha).getDay()] } : undefined, serieId: e.target.checked ? evento.id : undefined })} /> Repetir semanalmente</label>{recurrente && <><div className="weekday-picker">{DIAS.map((dia, index) => <label key={dia}><input type="checkbox" checked={evento.recurrencia!.dias.includes(index)} onChange={() => patch({ recurrencia: { ...evento.recurrencia!, dias: evento.recurrencia!.dias.includes(index) ? evento.recurrencia!.dias.filter((item) => item !== index) : [...evento.recurrencia!.dias, index] } })} />{dia}</label>)}</div><label>Hasta *<input type="date" min={evento.fecha} required value={evento.recurrencia?.hasta || ''} onChange={(e) => patch({ recurrencia: { ...evento.recurrencia!, hasta: e.target.value || undefined } })} /></label></>}</div>}<div className="drawer-actions"><button className="calendar-primary" onClick={() => onSave(evento)}>Guardar</button>{evento.tipo === 'Partido' && !evento.esNuevo && <button className="calendar-secondary" onClick={onOpenActa}>Abrir acta</button>}<button className="calendar-secondary" onClick={onClose}>Cancelar</button>{!evento.esNuevo && <button className="calendar-danger" onClick={onDelete}>Eliminar</button>}</div></aside></div>;
}
