...
import type { Acta, Competencia, EstadoPartido, Evento, InstanciaCopa, Prefs, TipoEvento } from '../lib/sportsCalendar';
...
export default function Calendario() {
  ...
  const saveEvent = (evento: Evento, scope: 'uno' | 'serie' = evento.ocurrenciaDe && window.confirm('¿Aplicar los cambios a toda la serie? Aceptar = toda la serie; Cancelar = solo este evento.') ? 'serie' : 'uno') => {
    ...
    const normalized = { ...evento, numeroFecha: isLeagueMatch ? evento.numeroFecha : undefined, instancia: isCupMatch ? evento.instancia : undefined, titulo: evento.titulo?.trim(), rival: evento.rival?.trim(), descripcion: evento.descripcion?.trim() };
  ...
  return <div className="sports-calendar-screen" style={{ animation: 'fadeIn .3s ease' }}>
    <header style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 18 }}>
      <div><h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#16203a' }}>Calendario</h1><p style={{ color: '#6b7488', margin: '4px 0 0' }}>Agenda de todos los planteles</p></div>
