...
type TipoEvento = 'Partido' | 'Entrenamiento' | 'Descanso' | 'Otro';
type Competencia = 'Liga' | 'Copa' | 'Amistoso';
type EstadoPartido = 'programado' | 'finalizado' | 'suspendido' | 'postergado';
type Recurrencia = { dias: number[]; hasta?: string };
type Evento = {
  id: number; equipoId: number; tipo: TipoEvento; fecha: string; horaInicio?: string; horaFin?: string;
  titulo?: string; descripcion?: string; rival?: string; condicion?: 'Local' | 'Visitante'; competencia?: Competencia;
  lugar?: string; estado?: EstadoPartido; motivo?: string; nuevaFecha?: string; recurrencia?: Recurrencia; serieId?: number;
  exclusiones?: string[]; overrides?: Record<string, Partial<Evento>>; ocurrenciaDe?: { id: number; fecha: string }; esNuevo?: boolean;
};
type Acta = {
  resultadoClub?: string; resultadoRival?: string; formacionId?: number; formacionSnapshot?: Formacion;
  goles: Array<{ jugadorId: string; minuto: string; asistenciaId: string }>;
  amarillas: Array<{ jugadorId: string; minuto: string }>; rojas: Array<{ jugadorId: string; minuto: string }>;
  cambios: Array<{ saleId: string; entraId: string; minuto: string }>; puntajes: Record<string, string>; observaciones: string;
};
type Prefs = { visibles: number[]; colores: Record<number, string>; configurado?: boolean };

const STORAGE = 'club-calendario-deportivo-v1';
...
const seedEventos: Evento[] = [
  ...
  { id: 102, equipoId: 1, tipo: 'Partido', fecha: '2026-08-02', horaInicio: '16:00', rival: 'Deportivo Belgrano', condicion: 'Local', competencia: 'Liga', lugar: 'Cancha principal' },
...
function emptyActa(): Acta { return { goles: [], amarillas: [], rojas: [], cambios: [], puntajes: {}, observaciones: '' }; }
...
export default function Calendario() {
  ...
  const [cursor, setCursor] = useState({ year: 2026, month: 7, day: 5 });
  const [vista, setVista] = useState<'mes' | 'semana'>('mes');
  const [eventos, setEventos] = useState<Evento[]>(seedEventos);
  const [actas, setActas] = useState<Record<string, Acta>>({});
  const [prefs, setPrefs] = useState<Prefs>({ visibles: [], colores: {} });
  const [editor, setEditor] = useState<Evento | null>(null);
  const [detalleDia, setDetalleDia] = useState<string | null>(null);
  const [actaEvento, setActaEvento] = useState<Evento | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved) as { eventos?: Evento[]; actas?: Record<string, Acta>; prefs?: Prefs };
        if (Array.isArray(parsed.eventos)) setEventos(parsed.eventos);
        if (parsed.actas) setActas(parsed.actas);
        if (parsed.prefs) setPrefs({ ...parsed.prefs, configurado: true });
      }
    } catch { /* datos locales inválidos: se usan las demostraciones */ }
    setHydrated(true);
  }, []);
  useEffect(() => { if (hydrated) try { window.localStorage.setItem(STORAGE, JSON.stringify({ eventos, actas, prefs })); } catch { /* almacenamiento no disponible */ } }, [eventos, actas, prefs, hydrated]);
  ...
  return <div className="sports-calendar-screen" style={{ animation: 'fadeIn .3s ease' }}>
    ...
    {actaEvento && <ActaEditor evento={actaEvento} acta={actas[String(actaEvento.id)] || emptyActa()} equipos={state.equiposDeportivos} jugadores={state.jugadores} formaciones={state.formaciones} onClose={() => setActaEvento(null)} onSave={(acta, finaliza, estado, motivo, nuevaFecha) => { const resultado = [acta.resultadoClub, acta.resultadoRival].map(Number); const puntajeInvalido = Object.values(acta.puntajes).some((valor) => valor !== '' && (Number(valor) < 1 || Number(valor) > 10)); if (finaliza && estado === 'finalizado' && ((acta.resultadoClub ?? '') === '' || (acta.resultadoRival ?? '') === '' || resultado.some((valor) => !Number.isInteger(valor) || valor < 0))) { actions.mostrarToast('Ingresá un resultado válido'); return; } if (puntajeInvalido) { actions.mostrarToast('Los puntajes deben estar entre 1 y 10'); return; } setActas((prev) => ({ ...prev, [String(actaEvento.id)]: acta })); if (finaliza) setEventos((prev) => prev.map((item) => item.id === actaEvento.id ? { ...item, estado, motivo, nuevaFecha } : item)); setActaEvento(null); actions.mostrarToast(finaliza ? 'Acta guardada' : 'Borrador guardado'); }} />}
  </div>;
}
...
function IncidentCard({ title, icon, action = '+ Agregar', onAdd, actions, children }: { title: string; icon: React.ReactNode; action?: string; onAdd?: () => void; actions?: React.ReactNode; children: React.ReactNode }) { return <section className="acta-incident-card"><header><div>{icon}<h3>{title}</h3></div>{actions || <button className="calendar-text-button" onClick={onAdd}>{action}</button>}</header>{children}</section>; }
